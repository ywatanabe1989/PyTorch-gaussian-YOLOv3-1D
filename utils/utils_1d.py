...
def bbox_w_iou(w1, w2): # w1: anchor
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    # Output tensors
    obj_mask = BoolTensor(nB, nA, nG).fill_(0)
    noobj_mask = BoolTensor(nB, nA, nG).fill_(1)
    class_mask = FloatTensor(nB, nA, nG).fill_(0)
    iou_scores = FloatTensor(nB, nA, nG).fill_(0)
    tx = FloatTensor(nB, nA, nG).fill_(0)
    tw = FloatTensor(nB, nA, nG).fill_(0)
    tcls = FloatTensor(nB, nA, nG, nC).fill_(0)
    ...
    return iou_scores, class_mask, obj_mask, noobj_mask, tx, tw, tcls, tconf
